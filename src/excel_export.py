...
def export_to_excel(data, filename):
    ...
    headers = ["Итого часов", "Итого в гроссе", "Тарифная ставка", 
               "Поощрение", "Удержания", "На карту", "Итого на руки"]
    ...
    for idx, employee in enumerate(data['rows'], 1):
        ...
        ws[f'AL{row_num}'] = employee.get('удержания', 0)
        ws[f'AM{row_num}'] = employee.get('на_карту', 0)
        ws[f'AN{row_num}'] = employee.get('итого_на_руки', 0)
    ...
    ws[f'AN{footer_row}'] = footer.get('итого_на_руки', 0)
    ...
    ws.column_dimensions['AL'].width = 13  # удержания
    ws.column_dimensions['AM'].width = 12  # на карту
    ws.column_dimensions['AN'].width = 16  # итого на руки
